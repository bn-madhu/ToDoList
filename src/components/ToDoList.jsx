...
const ToDoList = (props) => {
  let {taskName} = props;
  let [taskDetails, setTaskDetails] = useState(taskName);
  let [editBoolean, setEditBoolean] = useState(false);

  // edi the current task
  const editBtn = function(id){
    if(id === taskDetails.key){
      setEditBoolean(true)
      console.log("edit", taskDetails);
    }
  };

  const handelReEditing = function(event){
    console.log(taskDetails.titleName);
    setTaskDetails(taskDetails.titleName)
  }
  ...
  const completeBtn = function(e){
    console.log('complete', this);
  };

  return (
    <>
      <div className="todolist-wrapper" id={taskDetails.key+1}>
        <div className="task-list-container">
          {!editBoolean &&<h3 className='list-title'>{taskDetails.titleName}</h3>}
          {
            editBoolean && <input value={taskDetails.titleName} name="title" onChange={(e)=>{handelReEditing(e)}} />
          }
          { editBoolean && <Button theme="outline" onClick={()=>{setEditBoolean(false)}} className="secondary has-right-icon">Update</Button>
          }
          <p className='list-description'>{taskDetails.descritionName}</p>
        </div>
        <div>
          <Button onClick={() => editBtn(taskDetails.key)} theme="outline" className="secondary has-right-icon" id="edit-button">
            Edit
          ...
          <Button theme="outline" onClick={deleteBtn} className="primary has-right-icon" id="delete-button">
            Delete
          ...
          <Button onClick={completeBtn} className="success has-right-icon" id="complete-button">
            Complete
          </Button>
        </div>
      </div>
